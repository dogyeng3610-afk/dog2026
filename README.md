...
### 1. 개발 환경의 3대 요소
마이크로 컨트롤러를 활용한 임베디드 시스템 개발에는 다음의 도구들이 필요합니다.
* 컴파일러 (Compiler): 작성한 코드를 CPU가 이해할 수 있는 **기계어**로 변환합니다.
* 디버깅 툴 (Debugging Tool): 프로그램의 오류를 찾아내고 수정하는 도구입니다.
* ISP 툴 (In-System Programmer): 컴파일된 프로그램을 MCU 내부 메모리에 **적재(Download)**하는 장치입니다.

### 2. 통합 개발 환경: Atmel Studio 7.0
Atmel(현재 Microchip)사에서 제공하는 공식 소프트웨어로, 코드 작성부터 컴파일, 다운로드까지 하나의 프로그램에서 수행할 수 있습니다.
* 설치 경로: [Microchip 공식 홈페이지](http://www.microchip.com) 또는 이전 Atmel 사이트에서 다운로드 가능합니다.
* 특징: Visual Studio 기반의 인터페이스를 사용하여 사용자 편의성이 높습니다.


### 3. 프로젝트 생성 및 코드 작성 과정
1.  Project 생성: `File` → `New` → `Project` 메뉴를 선택합니다.
2.  템플릿 선택: `GCC C Executable Project`를 선택하여 C 언어 기반 프로젝트를 만듭니다.
3.  디바이스 선택: 사용하는 칩 모델인 **ATmega128A**를 정확히 검색하여 선택합니다.
4.  코드 입력: 생성된 `main.c` 파일에 제어 알고리즘을 작성합니다.

### 4. 컴파일 및 빌드 (Build)
* 방법: 상단 메뉴의 `Build` → `Build Solution`을 클릭하거나 단축키 **F7**을 누릅니다.
* 결과: 코드에 오류가 없다면 하단 출력창에 "Build succeeded" 메시지가 뜨며, 실행 파일인 **.hex** 파일이 생성됩니다.

### 5. 프로그램 다운로드 (Programming)
작성한 프로그램을 실제 하드웨어(Edge-MCU 등)에 넣는 단계입니다.
* 도구: **AVRISP mkII**와 같은 ISP 장비를 사용합니다.
* 설정: `Tools` → `Device Programming` 메뉴에서 Tool(AVRISP mkII), Device(ATmega128A), Interface(ISP)를 설정합니다.
* 실행: `Memories` 탭에서 생성된 `.hex` 파일을 선택하고 `Program` 버튼을 눌러 보드에 전송합니다.

### 6. 환경 설정 팁 (Troubleshooting)
* 드라이버 인식 문제: Windows 10 이상에서 AVRISP mkII가 인식되지 않을 경우, `zadig` 프로그램 등을 이용해 `libusb-win32` 드라이버로 교체해주어야 할 수도 있습니다.
* 툴 확인: 장치 관리자에서 'Atmel' 관련 항목 아래에 장치가 정상적으로 뜨는지 확인하는 것이 필수입니다.